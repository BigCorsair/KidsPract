...
+    () => {
+      const questions = [
+        createQuestion("1/2 + 1/4 = ? (fraction)", "3/4", { inputMode: "text", placeholder: "Example: 3/4" }),
+        createQuestion("5/6 - 1/3 = ? (fraction)", "1/2", { inputMode: "text", placeholder: "Example: 1/2" })
+      ];
...
+          <button key={grade} className="big-btn" onClick={() => { sounds.click(); onPick(grade); }}>
+            Grade {grade}
+          </button>
+        ))}
+      </div>
...
+      {bits.map((_, i) => (
+        <span key={i} className="confetti" style={{ left: `${(i / bits.length) * 100}%`, animationDelay: `${i * 0.03}s` }} />
+      ))}
+    </div>
+  );
 }
...
-});
+  if (screen === "home") {
+    return <GradeSelector onPick={(g) => { setGrade(g); setScreen("mode"); }} />;
+  }
+
+  if (screen === "mode") {
+    return (
...
+          ))}
+        </div>
+        <button className="ghost-btn" onClick={() => { sounds.click(); setScreen("home"); }}>
+          ← Back to grades
+        </button>
+      </div>
+    );
...
+        <button className="ghost-btn" onClick={() => { sounds.click(); nextQuestion(); }}>
+          Skip
+        </button>
+        <button className="ghost-btn" onClick={() => { sounds.click(); setScreen("mode"); clearInterval(timerRef.current); }}>
+          Change Mode
+        </button>
+        <button className="ghost-btn" onClick={() => { sounds.click(); setScreen("home"); clearInterval(timerRef.current); }}>
+          Home
+        </button>
+      </div>
